...
static void game_restart(u32 width, u32 height)
{
    ...
    stbsp_snprintf(game.score_buffer, sizeof(game.score_buffer), "Score: %u", game.score - game.delta);
}
...
void game_render(void)
{
    ...
    case STATE_GAMEOVER:
    {
        ...
        dead_snake_render();
        platform_fill_text(SCORE_PADDING, SCORE_PADDING, game.score_buffer, SCORE_FONT_SIZE, SCORE_FONT_COLOR, ALIGN_LEFT);
        platform_fill_text(game.width / 2, game.height / 2, "You are such a noob !", GAMEOVER_FONT_SIZE, GAMEOVER_FONT_COLOR, ALIGN_CENTER);
    }
    break;
    ...
    case STATE_SHOWFLAG:
    {
        ...
        const char *res = c;
        platform_fill_text(game.width / 2, game.height / 2, res, GAMEOVER_FONT_SIZE, GAMEOVER_FONT_COLOR, ALIGN_CENTER);
    }
    break;
...
#endif
                if (game.score == 0 && game.delta == 0)
                {
                    game.score = 5;
                    game.delta = 5;
                }
                int tmp = game.score % 5 + 1;
                game.score += tmp;
                game.delta += (tmp - 1);
                if ((game.score - game.delta) % 10 == 0)
                {
                    if ((game.score - game.delta) % 9 == 1)
                    {
                        if ((game.score - game.delta) % 7 == 2)
                        {
                            if ((game.score - game.delta) % 3 == 1)
                            {
                                game.step_cooldown = 0.0f;
                                game.state = STATE_SHOWFLAG;
                            }
                        }
                    }
                }
